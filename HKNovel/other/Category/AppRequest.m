...
//        [MBProgressHUD hideHUD];
        ...
        NSLog(@"url:%@ parameters_post   %@ ＝＝＝＝＝＝＝＝＝ responseObject%@",url,parameters,responseObject);
...
+(void)getRequestInURL:(NSString *)url andParameters:(NSDictionary *)parameters succes:(void (^)(id))success stateerror:(void (^)(id))stateerror error:(void (^)(NSError *))error1 controller:(UIViewController *)controller1
{
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    //    manager.responseSerializer.acceptableContentTypes = [NSSet setWithObject:@"application/json"];
    manager.requestSerializer.timeoutInterval = 5.f;
    ...
    [manager GET:url parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if (![state isEqualToString:@"0"]) {
            ...
            if ([state isEqualToString:@"0"]) {
                NSString *msg=[NSString stringWithFormat:@"%@", responseObject[@"desc"]];
                stateerror(responseObject);
                //            [GuBaseController mbProgressHUDModeTextstyle:MBProgressHUDModeStyle_text showview:controleer1.navigationController.view labletext:msg];
                    [MBProgressHUD showInfo:msg toView:controller1.navigationController.view];
            }else{
